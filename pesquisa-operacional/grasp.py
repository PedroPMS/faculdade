...
def add_arrow(line, direction='right', size=20, color='k'):
    ...
    if direction == 'right':
        ...
        e_idx = s_idx - 1
    line.axes.annotate('', xytext=(x[s_idx], y[s_idx]), xy=(
        x[e_idx], y[e_idx]), arrowprops=dict(arrowstyle='-|>', color=color), size=size)
    return

# Function: Tour Plot


def plot_tour(Xdata, city_tour=[], size_x=100, size_y=100):
    coordinates = 0
    no_lines = False
    ...
    if (no_lines == True):
        for i in range(0, xy.shape[0]):
            plt.plot(xy[i, 0], xy[i, 1], marker='s', alpha=1,
                     markersize=7, color='grey',  linestyle='None')
            plt.text(xy[i, 0], xy[i, 1], 'c-'+str(city_tour[0][i]))
    else:
        for i in range(0, xy.shape[0]-1):
            line = plt.plot(xy[i:i+2, 0], xy[i:i+2, 1], marker='s',
                            alpha=1, markersize=7, color='grey')[0]
            add_arrow(line)
            plt.text(xy[i, 0], xy[i, 1], 'c-'+str(city_tour[0][i]))
        line = plt.plot(xy[0:2, 0], xy[0:2, 1], marker='s',
                        alpha=1, markersize=7, color='red')[0]
        add_arrow(line, color='r')
        plt.plot(xy[1, 0], xy[1, 1], marker='s',
                 alpha=1, markersize=7, color='grey')
    plt.show()
    return
    ...
    for i in range(0, Xdata.shape[0]):
        ...
        if (rand > greediness_value and len(sequence) < Xdata.shape[0]):
            ...
            while next_city in sequence:
                ...
                next_city = int(
                    ranking(Xdata, city=sequence[-1] - 1)[count, 1])
            ...
            while next_city in sequence:
                next_city = int(random.sample(
                    list(range(1, Xdata.shape[0]+1)), 1)[0])
            sequence.append(next_city)
    sequence.append(sequence[0])
    seed[0] = sequence
    seed[1] = distance_calc(Xdata, seed)
...
def greedy_randomized_adaptive_search_procedure(Xdata, city_tour, iterations=50, rcl=25, greediness_value=0.5):
    ...
    while (count < iterations):
        ...
        for i in range(0, rcl):
            elemento = restricted_candidate_list(Xdata, greediness_value)
            rcl_list.append(elemento)
        ...
        while (city_tour[0] != rcl_list[candidate][0]):
            ...
            city_tour = local_search_2_opt(
                Xdata, city_tour=rcl_list[candidate])
...
rotas = dividirClientes.separarClientes(numVeiculos, clientes[:])
distanciaTotal = 0
for rota in rotas:
    ...
    seed = seed_function(arr)
    lsgrasp = greedy_randomized_adaptive_search_procedure(
        arr, city_tour=seed, iterations=5, rcl=10, greediness_value=0.5)

    distanciaTotal += lsgrasp[1]
print('Distance =', distanciaTotal)
